...
| A store's plugin was changed after review | The store entry pins a hash of the release archive; the installer verifies it before it extracts anything | built: the installer checks it (`verifyRelease`), and installs nothing that does not match |
...
| A different plugin version than the one approved gets loaded | The approval is of one exact hash; a new version needs a new approval, and a plugin whose approval does not fit is not imported | **built**, for updates and rollbacks (both withdraw the approval) |
| A plugin is "updated" from another store than the one it came from, or to an older or withdrawn version | The store is found from the plugin's `origin`, the client names none; only a newer, listed, not withdrawn version the store describes is offered; the release goes through the same checks as an install | **built** |
| A plugin update asks for more than the old one | The consent for an update says what the new version asks for; every added capability is in the audit entry, and the new code needs its own approval before it runs | **built**: the dialog sets apart what the new version asks for in addition ([The plugins page](admin.md#the-store)) |
| A rollback brings back files that are not the ones that were replaced, or a version the store withdrew | The hash of the old directory has to equal the one the row kept; the store's clone is asked whether it withdrew the version (any doubt allows it: the files need their own approval anyway) | **built** |
...
| A private store's access token leaks: from a database dump or backup, a log, an error, a page or an audit entry | Sealed with AES-256-GCM and bound to the store's address; never selected into a page, an action result, an error or an audit entry; only the store client may open it, and only to send it to that address ([Plugin stores](stores.md#private-repositories)) | built: stored sealed, and the store client opens it only for the store it was sealed for and sends it only to that host, never on after a redirect |
| A store's address or a plugin's download link, written by someone else, points at the instance's own network (a database, the cloud metadata service, an admin panel), or a redirect leads there | `safeDownload` refuses everything but https to a name whose every address is on the public internet, checks each redirect the same way, and limits size and time ([ADR 0003](adr-0003-store-transport.md)). **Not covered:** DNS rebinding, which needs the egress rules (BARY-97) | built, with the gap named |
| A store's archive is hostile: a path that leaves the directory, a symlink, a file that inflates to gigabytes, a tar that lies about a size | Our own tar reader refuses what it does not understand; only `store.json` and each plugin's `barynt-plugin.json` and `source.json` are written, everything else in the archive never reaches the disk ([ADR 0003](adr-0003-store-transport.md)) | built |
| A release archive is hostile or was swapped: a symlink, a name that leaves the directory, a zip that shows one thing to one reader and another to the next, a bomb, or a different release behind the same link | The SHA-512 the store pinned is checked before anything is read; the archive is read by our own readers and refused whole for a link, a bad name, a duplicate, a bomb, an ambiguous zip or anything over the limits of the plugin hash; the manifest inside has to equal the one the store lists; it is written into a fresh directory and moved in with one rename, never over what is there ([Release](release.md)) | built |
| A workspace admin or a project admin brings files onto the instance (the store for a workspace or a project, open by default): many plugins to fill the disk, a plugin for the whole platform or for the other level, a plugin with code that runs without the platform's say | Only from stores the platform switched on and, where it asked, only what it released; only a plugin that applies at their own level (a platform plugin, or one of the other level, is refused before any download); the same checks and hashes as the platform's own install; **the code of a plugin still needs the platform's approval for its exact files**, so what a workspace or a project adds is inert until then; every add is audited with the workspace or project that asked. The number of plugins is bounded by what the stores on offer | built; the platform can close it for workspaces and for projects separately ([The plugins of a workspace](workspace.md#the-store-of-a-workspace), [of a project](project.md#the-store-of-a-project)) |
...
**Who can add a plugin does not change who approves its code.** By default the store is shown in workspaces and projects too
([The plugins page](admin.md#who-gets-the-store)), so a workspace admin can add a plugin without asking. Adding puts files on the instance
and switches nothing on; code still runs only with the platform's approval for its exact files. The setting *only released plugins* narrows
what can be added, it never widens what can run.
