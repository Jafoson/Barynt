...
| A workspace admin brings files onto the instance (the store for a workspace, open by default): many plugins to fill the disk, a plugin for the whole platform, a plugin with code that runs without the platform's say | Only from stores the platform switched on and, where it asked, only what it released; only a plugin that applies per workspace (a platform plugin is refused before any download); the same checks and hashes as the platform's own install; **the code of a plugin still needs the platform's approval for its exact files**, so what a workspace adds is inert until then; every add is audited with the workspace that asked. The number of plugins is bounded by what the stores on offer | built; the platform can close it for workspaces ([The plugins of a workspace](workspace.md#the-store-of-a-workspace)) |
