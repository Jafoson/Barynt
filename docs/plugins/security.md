...
| A plugin update asks for more than the old one | The consent for an update says what the new version asks for; every added capability is in the audit entry, and the new code needs its own approval before it runs | **built** (the words in the dialog: BARY-108 UI) |
