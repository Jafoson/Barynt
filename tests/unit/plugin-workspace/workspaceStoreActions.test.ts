...
// A workspace adding a plugin from the store. What matters: `plugin.enable` in that workspace
// first; the request is checked and the risk is confirmed on the server; the store is only there
// where the platform gave workspaces one, and where the platform asked for it only for what it
// released; the install is the platform's (`installFromStore`, for a plugin that applies per
// workspace only, with the workspace that asked); switching on follows and, when it cannot yet
// (the platform has not approved the code), that is a warning and not a failure of the add.
...
const mockEnable = mock(
  async (_ws: string, _id: string): Promise<unknown> => ({ ok: true }),
);
...
const add = (
  ...
  addStorePluginToWorkspace(
    (more.ws ?? "ws-7") as string,
...
describe("who may", () => {
  it("asks for plugin.enable in the workspace of the request, first, and never for plugin.manage", async () => {
    await add();
  ...
  it("does not even ask for a workspace that is not text", async () => {
    for (const ws of [42, "", "w".repeat(101), undefined]) {
      expect(
...
describe("what the platform allowed", () => {
  it("is nothing when the store is not shown in workspaces", async () => {
    mockSettingsFindUnique.mockResolvedValue({
      ...OPEN,
      pluginStoreInWorkspaces: false,
    });
    expect(await add()).toEqual({
      error: "The plugin store is not available in workspaces.",
    });
    nothingDone();
  });
...
describe("the install", () => {
  it("is the platform's, for a plugin that applies per workspace, on behalf of this workspace and this person", async () => {
    await add();
  ...
  it("takes nothing from the client but the ids and the yes", async () => {
    await addStorePluginToWorkspace("ws-7", "store-1", "notes", "1.0.0", {
      acknowledged: true,
      source: "STORE",
...
describe("the switch after it", () => {
  it("is in this workspace, for this plugin, once it is installed", async () => {
    expect(await add()).toEqual({ ok: true });
  ...
  it("is a warning, not a failure, when it cannot be switched on yet: the plugin is added", async () => {
    ...
    expect(await add()).toEqual({
      ok: true,
      warning:
        "notes was added, but it is not switched on in this workspace yet: The platform has not approved its code.",
    });
  });

  it("passes on a warning of the switch", async () => {
    mockEnable.mockResolvedValue({ ok: true, warning: "onEnable was slow" });
    expect(await add()).toEqual({ ok: true, warning: "onEnable was slow" });
  });
});
