...
- Plugins are read from `BARYNT_PLUGINS_DIR` (absolute path, ideally outside the app
  ...
  `lib/plugins/policy.ts` (`decideExecution`): only a plugin with code from the official store that
  the platform approved for its exact hash; everything else with `server` or `client` is blocked,
  and any doubt means blocked.** The loader is only ever given plugins that policy lets through.
